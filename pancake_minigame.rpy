...
label win:
    ...
    "Congratulations!"
    # TODO make this move to proper point in chapter_1
    jump chapter_1
...
init python:
    ...
    class Table(renpy.Displayable):
        ...
        def get_card(self, size):
            if size not in self.cards:
                raise Exception("No card has the size %r." % size)
        ...
        def card(self, size, back=None):
            self.cards[size] = __Card(self, size, back)
        ...
        def event(self, ev, x, y, st):
            ...
            if ev.type == pygame.MOUSEBUTTONDOWN and ev.button == 1:
                ...
                if card is None or not self.can_drag(self, card.stack, card.size):
                    self.drag_cards = [ ]
            ...
            if ev.type == pygame.MOUSEBUTTONUP and ev.button == 1:
                ...
                if self.dragging:
                    if dststack is not None and self.drag_cards:
                        ...
                        evt.card = self.click_card.size
                        evt.drag_cards = [c.size for c in self.drag_cards]
                        evt.drop_stack = dststack
                        if dstcard:
                            evt.drop_card = dstcard.size
                        evt.time = st
    ...
    class __Card(object):
        ...
        def __init__(self, table, size, back):
